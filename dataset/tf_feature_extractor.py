...
MODEL_DIR = os.path.join(os.getenv('HOME'), 'y8_tf/model')
...
class YouTube8MFeatureExtractor(object):
  ...
  ** Note: OpenCV reverses the order of channels (i.e. orders channels as BGR
  ...
  def __init__(self, model_dir=MODEL_DIR,pca_dir = ''):
    # Create MODEL_DIR if not created.
    self._model_dir = model_dir
    self._pca_dir = pca_dir
    ...
    pca_mean = os.path.join(pca_dir, 'mean.npy')
  ...
  def _load_pca(self):
    self.pca_mean = numpy.load(
        os.path.join(self._pca_dir, 'mean.npy'))[:, 0]
    self.pca_eigenvals = numpy.load(
        os.path.join(self._pca_dir, 'eigenvals.npy'))[:1024, 0]
    self.pca_eigenvecs = numpy.load(
        os.path.join(self._pca_dir, 'eigenvecs.npy')).T[:, :1024]

if __name__ == '__main__':
  yt8m = YouTube8MFeatureExtractor()
  image_file = os.path.join(MODEL_DIR,'cropped_panda.jpg')
  im = numpy.array(Image.open(image_file))
  feature = yt8m.extract_rgb_frame_features(im)